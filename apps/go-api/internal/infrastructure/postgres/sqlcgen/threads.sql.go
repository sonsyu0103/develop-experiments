...
const listMyThreadsWithCommentCount = `-- name: ListMyThreadsWithCommentCount :many
WITH page AS (
...
type ListMyThreadsWithCommentCountParams struct {
	ActorID  *int64
	CursorID *int64
	PageSize int32
}

type ListMyThreadsWithCommentCountRow struct {
	ID                int64
	Title             string
	CreatedAt         time.Time
	ViewCount         int64
...
func (q *Queries) ListMyThreadsWithCommentCount(ctx context.Context, arg ListMyThreadsWithCommentCountParams) ([]ListMyThreadsWithCommentCountRow, error) {
	rows, err := q.db.Query(ctx, listMyThreadsWithCommentCount, arg.ActorID, arg.CursorID, arg.PageSize)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListMyThreadsWithCommentCountRow{}
	for rows.Next() {
		var i ListMyThreadsWithCommentCountRow
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.CreatedAt,
