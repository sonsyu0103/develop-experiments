...
const (
	...
	// **バイト数の上限を守ったまま到達できます**。
	//
	// **1 枚あたりの見積もりは、以前ここに書いていた表の約 4.8 倍です。**
	// 「RGBA で 4 バイト/画素」で数えていたのが誤りでした (2026-08-23 実測):
	//
	//	2500 万画素   456 MB / 枚 (実測)   ← 表には 95 MB と書いてあった
	//
	// 16bit PNG は image.NRGBA64 (8 バイト/画素) になり、
	// 縮小の一時バッファ (x/image/draw の CatmullRom) が別に要ります。
	// **式は usecase 側の estimatedPeakBytesPerDecode にあります**
	// —— 数字をコメントに置くと、ずれても誰も落とせないため。
	//
	...
	// 実測後、その上限は 1 本になりました。**ここを下げれば戻せます**が、
	// 受け付ける入力が変わるので ADR 0007 決定 2 の再改訂になります (未決)。
