...
func Load() (*Config, error) {
	...
	// 実測すると、スループットは上限 8 で頭打ちになり、それ以上増やしても
	// 伸びずに**遅い側の応答だけが悪化する** (4 vCPU / 並列 64 で測定):
	//
	//	上限  8 : 2,157 RPS / p95  36 ms
	//	上限 20 : 2,073 RPS / p95  45 ms   ← 以前の既定
	//	上限 64 : 2,184 RPS / p95 122 ms
	//
	// 上限を増やすと、待つ場所がアプリの待ち行列から DB の中へ移る。
	// DB の中で待たれると全員が一緒に遅くなるので、
	// **手前で絞ったほうが遅い側の応答が守られる。**
	// pgbench で DB 単体を測った最適点 (接続数 = コア数の 2 倍) とも一致した。
	//
