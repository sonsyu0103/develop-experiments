...
type Config struct {
	...
	//
	// **DB のコア数に対して決める値です** (既定 8 は 4 vCPU での実測から)。
	// タスク数を増やすときは、全タスクの合計がこの値の何倍になるかを見ること
	// (docs/adr/0009-scaling-strategy.md の「実測」)。
	MaxConns int32
...
type MailConfig struct {
	...
	// このシステムを踏み台にできます。
	//
	// **ここが唯一の宛先ではありません。** ログイン済みの問い合わせには、
	// users.email (IdP が検証済み) 宛にも控えを 1 通送ります。
	// そちらは設定値ではなく、送信のたびに DB から引き直します。
	To string
...
func Load() (*Config, error) {
	...
	//
	// 【既定を 20 から 8 に下げた (Phase 4 / make scale-probe)】
	// 20 は測らずに置いた数字で、**DB のコア数を知らないまま決めていた。**
	//
	// **スループットでは決まらない。** 上限を 4 から 64 まで振っても
	// 最良 RPS は 1.15 倍にしかならず、実行ごとのばらつきと同程度になる。
	// 一貫して効くのは**遅い側の応答**のほう
	// (4 vCPU / 並列 64、p95):
	//
	//	上限  8 : 2,158 RPS / p95  35 ms
	//	上限 20 : 2,198 RPS / p95  41 ms   ← 以前の既定
	//	上限 64 : 2,353 RPS / p95 110 ms
	//
	// 上限を増やすと、待つ場所がアプリの待ち行列から DB の中へ移る。
	// DB の中で待たれると全員が一緒に遅くなるので、
	// **手前で絞ったほうが遅い側の応答が守られる。**
	//
	// 8 にする根拠は 3 つを重ねたもの:
	//   1. pgbench が示した DB 単体の最適点 (接続数 = コア数の 2 倍) と一致
	//   2. p95 が悪化し始める手前 (16) より小さい
	//   3. **1 タスクぶん**の値なので、タスクを増やしたときの合計に余裕がある
	//
	// **8 が普遍的に正しいわけではない。** 適正値は DB のコア数に比例し、
	// アプリからはそれが見えない。固定値である以上どの数字も推測になるが、
	// 8 は外し方が安全な側に倒れている ——
