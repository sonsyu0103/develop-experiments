...
項目数は環境によって変わる。Google の資格情報がある手元では **79 項目**、
...
| Phase 2 | コメント投稿の並行制御強化 (SSI + リトライ、悲観ロック版との比較、冪等キー) | レス番号と 4 実装の比較まで完了。冪等キーが残り |
...
2. **Phase 2 (並行制御)** —— 主題。スキーマが固まってから作り込む。
   **レス番号 (`comments.seq`) を題材に 4 実装を比較するところまで完了**
   ([ADR 0019](docs/adr/0019-comment-concurrency.md))。
   残りは冪等キー ([ADR 0015](docs/adr/0015-idempotency.md))
