...
| | 天井 |
| --- | --- |
| 負荷生成 + HTTP (`/healthz`) | 33,805 RPS |
| 本番の読み取り経路 (`/threads`) | **2,262 RPS** |
| PostgreSQL 単体 (pgbench / 同じ SQL) | 3,570 TPS |

`/threads` は校正の 7% で頭打ちになる。生成側にはまだ 14 倍の余裕があり、
**2,200 RPS は測定器の限界ではなく API 側の限界**になる。
`docker stats` で見ると、頭打ちの位置では
**postgres だけが 260% (4 コア中) で張り付き、go-api は 63% で余っている**。

接続数を振ると、**スループットは接続数 = コア数の 2 倍でピークになり、
そこから下がる** (4 vCPU で 8 接続が最適、64 接続では 13% 落ちる)。

**この結果で `DB_MAX_CONNS` の既定を 20 から 8 に下げた。**
API 経由でも折り返しは同じ位置に出た ——
上限を 8 から 64 まで増やしてもスループットは 2,100 前後で動かず、
**変わるのは遅い側の応答だけ**だった (並列 64 での p95 が 36 ms → 122 ms)。
...
| **4,200 RPS そのもの** | 上の実測は 4 vCPU での 2,262 RPS まで。8 vCPU で 2 倍出るとは限らない (接続数の最適点も動く) |
