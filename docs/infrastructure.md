...
- 日付: 2026-08-05
- ステータス: **理想構成の設計ドキュメント。一部は Terraform で実装済み** (下記)

## この文書の位置づけ

**もともとは、デプロイの計画ではなく実装の指針として描いたもの。**

「本番に出すならどこに置くか」を決めておかないと、
実装中の細かい判断 (画像をどこに置くか、秘密情報をどう読むか、
どのプロセスが外向き通信をするか) が場当たりになるため、先に図にした。

**2026-08-25 に、この図の一部を Terraform で実装した**
([ADR 0024](adr/0024-aws-deployment.md))。ただし**常時公開はしない** ——
見せたい日に `make tf-apply` して、終わったら `make tf-destroy` する運用になる。
実装した範囲と、図のまま残っている範囲は下の表のとおり。
...
| リードレプリカ / Multi-AZ | 図のまま (ADR 0009 のとおり、負荷を測ってから) |
| SES / Google OIDC | Terraform 側の口は用意済み。**未設定**
...
`MaxConns` の既定は Phase 4 の実測で 20 から 8 に下げたが、
**タスク数に比例する問題そのものは残る** (30 タスクで 240 接続)。
