# ADR 0004: モジュラモノリスとレイヤ境界を lint で強制する

- ステータス: **採用** (検査ツールの決定のみ [ADR 0017](0017-arch-lint-and-depguard.md) が置き換え)
- 日付: 2026-08-03 / 更新: 2026-08-06

> **この ADR の「決定」は 2 つに分かれる。**
>
> - **モジュールの切り方と結線方式** (必要な操作だけのインターフェースを
>   利用側が定義し、`cmd/api` で実装を注入する) —— **有効**
> - **検査ツールの選択 (B: `depguard`)** ——
>   [ADR 0017](0017-arch-lint-and-depguard.md) が `go-arch-lint` に置き換えた。
>   下記の `.golangci.yml` のルール表と `files` の落とし穴は、
>   **現在のコードには存在しない**。当時の判断の記録として残している
...
> **この評価は誤りだった** ([ADR 0017](0017-arch-lint-and-depguard.md))。
> 差は「厳密さ」ではなく、**宣言し忘れたパッケージが無検査になるか違反になるか**にある。
> 2 モジュールの時点では表面化しないが、モジュールを足すたびに効いてくる。
...
> このプローブは手作業だったが、[ADR 0017](0017-arch-lint-and-depguard.md) で
> 16 件に拡張して `.github/scripts/arch-probe.sh` として CI に載せた。
> 下表は当時の `depguard` 設定に対する結果になる。
...
- ~~モジュールを増やすたびにルールが 2 つ増える。
  ...
  **破綻した。** モジュールが 6 つになる見込みになり
  ([ADR 0011](0011-moderation.md))、3 モジュール目に入る前に
  [ADR 0017](0017-arch-lint-and-depguard.md) で選択肢 C に移行した
