...
> **管理画面 (2026-08-16) では 1 層で済んだ。**
>
> [ADR 0013](0013-http-defense.md) が「書き込みはブラウザから直接叩く」を
> 既定にしているので、管理画面は**読み取りもブラウザへ寄せた**。
> その結果、**サーバが返す HTML と RSC ペイロードに利用者ごとの内容が
> 1 つも入らない。**
>
> 確認すべき層は `fetch` の `cache: 'no-store'` だけになる。
> ルートを動的にする必要も、CDN で管理画面のパスを避ける必要もない ——
> 静的なまま配っても中身が空だからになる。
>
> **実測**: `next build` の出力で `/admin` と `/admin/roles` は
> `○ (Static)` になった。リクエストに依存する入力が 1 つも無い、
> というビルド時の判定になる。
>
> 逆に言うと、**管理画面で `cookies()` を読んだ瞬間に 4 層へ戻る。**
> 「サーバ側で認証を見て出し分ける」は自然な発想だが、
> この画面ではそれが最も危ない選択になる。
> ロールによる出し分け (`Me.role`) も Client Component で行っている。
...
> **2026-08-23 追記: 下の表に入っていない経路が 3 つある。**
> コールバックの失敗を JSON からリダイレクトへ寄せた際に足した
> `login_expired` / `state_mismatch` / `no_code`、および交換より後ろの失敗の
> `login_failed` は、**実サーバでは確認していない** (単体検査のみ)。
> 「拒否された場合も実サーバで確認した」の節が扱っているのは
> `error=access_denied` の経路だけになる。
