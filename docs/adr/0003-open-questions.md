...
決まるまでは未使用のまま残す。行き先が分かるよう、
`CommentRepository.SoftDelete` と `db/query/comments.sql` の双方から
この項目を参照している。

なお `LockThreadForUpdate` (Phase 2 の悲観ロック版で使用) と
`FetchThreadListNPlusOne` (Phase 4 のベンチマークで使用) も現時点では
未使用だが、こちらは用途がコードとロードマップに明記されている。
