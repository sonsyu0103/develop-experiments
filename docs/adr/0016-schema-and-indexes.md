...
| --- | --- | --- | --- |
| 1 スレッドのコメント一覧 | **1.07** | 1.70 | 2.13 |
| 20 スレッドぶんの集計 (除外が効かない) | **1.29** | 5.44 | 9.75 |
| 時刻で絞る全体集計 (除外が効かない) | **142.5** | 202.3 | 211.5 |
| 1 行 INSERT | **0.86** | 1.27 | 1.50 |

**「桁で違う」という上の記述は、この規模では過大だった。**
除外が効かないクエリで分割が負けるのは事実だが、
200 万行では 4.2 倍 (20 スレッドの集計) / 1.4 倍 (全体集計) で、
桁は変わっていない。各パーティションでの索引スキャンが効くためになる。

**負けの内訳はほとんど計画時間になる。** 20 スレッドの集計では
HASH 8 の 5.44 ms のうち 3.78 ms が計画で、実行は 1.66 ms しかない。
