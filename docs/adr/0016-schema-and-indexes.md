...
- ステータス: **採用 (一部実装)**。マイグレーション `000001`〜`000014` まで反映済み。
  ただし**問題 2 の決定は実装時に反転している** (下記。`comments_id_idx` は作られず、
  棄却したはずの選択肢 A が入った)
- 日付: 2026-08-06
...
> **この決定は実装で反転した (2026-08-19 に確認)。**
> `comments_id_idx` はどのマイグレーションにも存在しない。
> 実際に入ったのは**棄却したはずの選択肢 A** のほうになる。
>
> | 参照元 | 実装 |
> | --- | --- |
> | `reports` | `000009` が `target_thread_id BIGINT` を追加。`reports_thread_id_matches_target` で `target_type` との整合を縛る |
> | `moderation_actions` | `target_id` (TEXT) に `"{thread_id}:{comment_id}"` を詰める (`formatCommentTarget`) |
>
> **どちらも `(thread_id, id)` で引ける形になったので、`comments (id)` の
> 索引は不要になった。** 上の「8 回のインデックスシークは許容できる」という
> 判断そのものが要らなくなっている。
>
> 反転の理由は `000009` のコメントに残っている ——
> `moderation_actions` は `target_id` が TEXT なので複合キーを詰められたが、
> `reports` は BIGINT で、決定 4 が「型が混ざらないので揃えて TEXT にはしない」
> と決めていた。**列を足すほうがその決定と噛み合う。**
>
> 「A を棄却して B を採る」と書いたまま A が入っている状態は、
> **索引がある前提でモデレーション経路のクエリを書かせる**。
> 決定を消さずに、反転したことを残す。
...
初版は「自分のスレッド 先頭ページ **0.37 ms / 18 / — (分割なし)**」と
書いていた。**プローブが出荷するクエリではなく、素の走査を測っていたため。**

出荷する `ListMyThreadsWithCommentCount` はコメント数の相関サブクエリと
`users` / `images` の LEFT JOIN を含む。実物で測り直すと 3 倍になり、
**「分割なし」も誤りだった** —— threads 自体は分割していないが、
**コメント数の相関サブクエリが `comments` を引くので 8 区画すべてを触る。**

「マイページのスレッド一覧は区画を跨がない」という理解自体が誤りで、
**跨がないのは `threads` を読む部分だけ**になる。
プローブは実クエリに合わせ直した (`.github/scripts/query-probe.py`)。

**「桁で違う」は当たっていた。** 1.52 ms 対 0.16 ms で 9.5 倍、
...
| `000013` | `images` の CHECK 制約の修正 (`pending` を削除可能に) | 10 後半 | — |
| `000014` | **誰も書かない `threads.updated_at` の削除** (DB レビュー) | — | — |
