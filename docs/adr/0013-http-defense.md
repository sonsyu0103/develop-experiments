...
- ステータス: **採用 (実装済み)**。決定 1 (CSRF) と決定 3 (エラーコード) は
  2026-08-14。**決定 2 (セキュリティヘッダ) は 2026-08-24 に実装** ——
  ただし go-api のミドルウェアではなく**エッジで付ける**形になった
  ([ADR 0023](0023-local-edge-and-https.md) 決定 4)。CSP は引き続き未実装
- 日付: 2026-08-05
...
X-Frame-Options: DENY            (エッジで全応答に付ける。ADR 0023 決定 4)
...
**壊れ方は想定より広い。** HSTS は**ホスト単位で記憶され、ポートを区別しない**ため、
`localhost` に付けると `http://localhost:3000` も `http://localhost:8080` も
以後すべて https へ強制される —— エッジを使わない経路まで巻き添えになる。
エッジ (ADR 0023) の既定は `max-age=0` (記憶するな / 記憶済みなら忘れろ) にしてある。
...
> **管理画面 (2026-08-16) でこの既定に従った。**
> 書き込みだけでなく**読み取りもブラウザへ寄せた** ——
> サーバ側から読むと Cookie が自動転送されず
> ([ADR 0005](0005-authentication.md))、読みと書きで経路が分かれるほうが
> 間違えやすいため。副産物として ADR 0005 の 4 層キャッシュが
> 1 層に減った (そちらに記録)。

### 8. **`Allow-Methods` が 2 度ずれた。目視で揃えるのをやめた**

6 で `PUT` の欠落を直したのと**同じ故障が、同じ場所でもう一度起きた。**
通報とロール変更を足した PR が `PATCH` を、本人削除を足した PR が
`DELETE` を、それぞれ `cors()` に反映していなかった。

| | 呼べなくなっていたもの |
| --- | --- |
| `PATCH` | `PATCH /moderation/reports/{id}` / `PATCH /users/{publicId}/role` |
| `DELETE` | `DELETE /threads/{id}` / `DELETE /threads/{id}/comments/{id}` |

**Phase 10 後半で足した書き込み系が、ほぼ全滅していた。**
気づいたのは管理画面を書き始めたときで、それまでの検査は
**全部通っていた** —— ブラウザはこの応答を見て本リクエストを諦めるので、
**サーバには何も届かず、ログにも痕跡が残らない。**
API 単体の検査が届かない場所になる。

6 の対処は「仕様書にメソッドを足したら `cors()` も見ること」だったが、
**それは守られなかった。** 人が覚えている前提の対処は 2 度目で破れる。

`corsAllowedMethods` を定数にし、**仕様書そのものと突き合わせる検査**を
足した (`TestCORSAllowedMethods_AgreesWithSpec`)。
`GetSwagger()` が返す各パスの操作を集め、
許可メソッドと過不足なく一致することを見る。
スモークにも同じ検査を置いた —— こちらは実際の preflight 応答を見るので、
定数と実際の応答がずれる形も捕まる。

どちらも**戻して実測した**。単体検査は
「`DELETE` が無い —— `/threads/{threadId}` を呼べない」と
パスまで挙げて落ち、スモークは 2 件落ちる。
