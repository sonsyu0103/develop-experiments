...
`infra/duckdb/logs_source.py` が **Athena の DDL を読んで、
欠けている列を NULL 列として補う**。DDL が列の単一の情報源であり、
写経しないことで「DDL に足した列を片方だけ足し忘れる」を防いでいる。

副産物として、**DDL に宣言されていない列がデータに出ている**ことを
検出できる。宣言漏れは Athena では静かに NULL になるだけで、
